...
for epoca=1:epochmax
    ...
    elseif epoca>1
        ...
        for i=1:m,  
            for j=1:p,
                if DW(i,j)<0,       
                    TAWji(i,j)=min([TAWji(i,j)*TA1 TAMAX]); 
                elseif DW(i,j)>0,   
                    TAWji(i,j)=max([TAWji(i,j)*TA2 TAMIN]);       
                end
                if DW(i,j)<=0,      
                    deltaWji(i,j)=-TAWji(i,j)*(gWji(i,j));
                else, 
                    gWji(i,j)=0;  deltaWji(i,j)=-deltaWji0(i,j);
                end
            end,
        end
        ...
        for i=1:q,  
            for j=1:r,
                if DV(i,j)<0,       
                    TAWkj(i,j)=min([TAWkj(i,j)*TA1  TAMAX]); 
                elseif DV(i,j)>0,  
                    TAWkj(i,j)=max([TAWkj(i,j)*TA2 TAMIN]);       
                end
                if DV(i,j)<=0,      
                    deltaWkj(i,j)=-TAWkj(i,j)*(gWkj(i,j));
                else, gWkj(i,j)=0;  
                    deltaWkj(i,j)=-deltaWkj0(i,j);
                end
            end,
        end
        Wkj=Wkj+deltaWkj;
